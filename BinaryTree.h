...
public:
	...
	bool BuildTree(bool order, std::string s1, std::string mid) {
		tag = true;
		if(order == 0) {
			root = preOrder(s1, mid, nullptr, 0);
...
private:
	...
	void preShow(BinTreeNode<T> *rt) {
		...
		std::vector<std::pair<char, std::pair<int, int>>> tmp;//保存一行的元素值
		...
		while(!q.empty()){
			...
			q.pop();
			tmp.push_back(std::make_pair(u->data, std::make_pair(0, 0)));
